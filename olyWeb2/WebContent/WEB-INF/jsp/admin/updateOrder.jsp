<%@include file="../include/include.jsp"%>
			...
			<c:choose>
				<c:when test="${not empty cart.order.orderId}">
					...
					<div id="cart_contents"><img src="<c:url value="/images/spinner.gif"/>"/></div>
						...
						<p>
							<a class="button" href="<c:url value="/admin/getOutstanding.oly"/>">Outstanding orders</a>
							<a class="button" href="<c:url value="/admin/admin.oly"/>">Admin page</a>
							<a class="button" href="<c:url value="/j_spring_security_logout"/>">Log out</a>
						</p>
				</c:when>
				<c:otherwise>
					<h2>Order Not Found</h2>
					<p>Can't find that order. Go back to the <a href="admin.oly">main page</a> and try again</p>
				</c:otherwise>
			</c:choose>
		</div>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				$.get("<c:url value="/async/cartTable.oly"/>?order_id=${cart.order.orderId}&src=checkout", function(data) {
