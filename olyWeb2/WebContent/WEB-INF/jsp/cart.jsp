<%@include file="include/include.jsp"%>
			...
			<h2>Shopping Cart</h2>
				<c:choose>
					<c:when test="${fn:length(cart.orderProducts) eq 0}">
						<p>There are no items in your shopping cart. Head over to the <a href="products.oly">Product list</a> to find and add some!</p>
					</c:when>
					<c:otherwise>
						<p>
						You have ${fn:length(cart.orderProducts)} item<c:if test="${fn:length(cart.orderProducts) gt 1}">s</c:if> in your cart:</p>
						<form method="post" action="updateCart.oly" onsubmit="return validateUpdateCart();">
				...
				<a class="button" href="products.oly">Continue shopping</a>
				<c:if test="${fn:length(cart.orderProducts) gt 0}">
					<a class="button" href="checkout.oly">Proceed to checkout</a>
				</c:if>
			<p>
		</div>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$.get("async/cartTable.oly?src=cart", function(data) {
